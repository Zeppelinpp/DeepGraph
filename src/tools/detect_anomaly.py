...
# --- 3. 核心调整：异常分析与自然语言查询生成 ---
def source_anomaly(all_analyses_text:str) -> Optional[Dict[str, Any]]:
    """
    分析财务指标异常，仅生成【业务层面的自然语言查询语句】（无数据库语法）
    """
    # 确保输入是字符串（若传入字典则先序列化）
    if isinstance(all_analyses_text, dict):
        all_analyses_text = json.dumps(all_analyses_text, ensure_ascii=False)
    
    prompt = f'''
你是一名资深财务数据分析师，负责从输入的财务指标分析结果中识别异常，并生成【自然语言查询语句】（描述需要进一步核查的业务内容，无需任何数据库语法），用于定位数据来源，查明数据异常的真实原因。

## 核心规则
1. 仅处理状态为"success"的指标，忽略其他状态；
2. 异常识别严格遵循下方“异常分类规则”，不扩展、不引申；
3. 仅针对识别出的异常数据生成查询语句，无异常则明确说明“数据波动合理，不存在异常情况”；
4. 自然语言查询需清晰描述：待核查的科目名称、时间范围、需要确认的具体信息（如金额、备注、原因等）。
...
## 输出格式要求
1. 输出为JSON字典，仅包含"anomaly_query_result"键，值为列表(数组)（每个元素对应1个异常的查询描述）；
2. 列表元素说明："anomaly":"简要说明异常类型和波动幅度","nl4nebula":"使用自然语言描述需要核查的业务内容，用于访问nebula数据库"；
3. 根据异常查询语句，产生自然语言查询需要包括异常数据本身，数据的所有相关凭证中的摘要）
4. 无异常时，输出："anomaly_query_result":[]
5. 避免任何多余文字，仅返回JSON。
...
## 输出示例（正确格式）
    ...
    {{
      "anomaly_query_result": [
        {{
            "anomaly": "2024年12月管理费用为21000，较11月16500环比增长27%，超过15%的异常阈值",
            "anomaly_source": "管理成本表"
            "anomaly_type": "环比波动异常",
            "nl4nebula": "获取2024年12月管理费用的所有凭证摘要及金额"
        }}
      ]
    }}
    ```
...
## 错误示例（需避免）
- 包含"NEBULA"、"MATCH"、"RETURN"等数据库关键字；
- 未明确时间范围或科目名称；
- 生成无异常数据的查询语句。
'''
    # 确保输入是字符串（若传入字典则先序列化）
    ...
    # full_prompt = prompt.format(all_analyses_text=all_analyses_text)
    result = call_llm_api(prompt=prompt)
    
    # 统一输出格式（处理无结果或格式异常的情况）
    if not result or "anomaly_query_result" not in result:
        logging.info("未识别到异常数据，返回无异常提示。")
        return {
            "anomaly_query_result": []
        }
    return result
...
if __name__ == "__main__":
    try:
        ...
        for task in analysis_tasks:
            ...
            anomaly_query = source_anomaly(result)
